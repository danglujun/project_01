window.addEventListener('load', function () {
	...
	for (var i = 0; i < ul.children.length; i++) {
		...
		li.addEventListener('click', function () {
			// 干掉所有人，清除所有li的current类名
			for (var i = 0; i < ol.children.length; i++) {
				ol.children[i].className = '';
			}
			// 留下我自己，给当前的li添加current类名
			this.className = 'current';
			// 5.点击小圆圈，移动图片，注意移动的是ul
			// ul的移动距离=小圆圈的索引号*图片的宽度，注意是负值
			// 当我们点击了某个li，就拿到当前li的索引号
			var index = this.getAttribute('index');
			// 当我们点击了某个li，要将这个li的索引号给num
			num = index;
			// 当我们点击了某个li，要将这个li的索引号给circle
			circle = index;
	...
	arrow_r.addEventListener('click', function () {
		if (flag) {
			...
			circle++;
			// 如果circle==4，说明走到最后克隆的一张图片了，就将circle复原为0
	...
	arrow_l.addEventListener('click', function () {
		if (flag) {
			flag = false;
			// 如果走到了第一张图片，此时ul要快速跳转至最后克隆的一张图片
			if (num == 0) {
			...
			// 点击左侧按钮，小圆圈跟随一起变化
			circle--;
			// 如果circle<0，说明走到第一张图片了，就将circle改为最后一个小圆圈
			/* if (circle < 0) {
